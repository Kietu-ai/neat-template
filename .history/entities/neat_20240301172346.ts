...
export class Neat {
  ...
  private performCrossover(parent1: Genome, parent2: Genome) {
    ...
    tf.tidy(() => {
      ...
      const parentHiddenNodes = parent1.getBrain().getHiddenNodes(); // Number of hidden nodes
        ...
        for (let k = 0; k < mutableValues.length; k++) {
          for (let j = 0; j < mutableValues[k].length; j++) {
            if (Math.random() < this.mutationRate) {
              const offset = Math.random() * 2 - 1; // Adjust the range as needed
  ...
  public evolveNextGeneration(): void {
    ...
    //saved organisms keeps a copy of the population
    //even when we set it into an empty arrays, the neural networks are still in memory
