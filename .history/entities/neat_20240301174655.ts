...
export class Neat<T extends ControllableEntity> {
  private population: Genome<T>[];
  private generation: number;
  private populationSize: number;
  private mutationRate: number;
  private savedOrganisms: Genome<T>[];
  ...
  private selectParents(): { parent1: Genome<T>; parent2: Genome<T> } {
  ...
  private performCrossover(parent1: Genome<T>, parent2: Genome<T>) {
    const newPopulation: Genome<T>[] = [];
    ...
    tf.tidy(() => {
      ...
      for (let i = 0; i < this.populationSize; i++) {
        ...
        const child = new Genome<T>(childBrain);
  ...
  private mutate(child: Genome<T>): Genome<T> {
  ...
  public evolveNextGeneration(): void {
    ...
    const mutatedPopulation: Genome<T>[] = [];
  ...
  public removeOrganism(organism: Genome<T>): void {
