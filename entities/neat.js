...
class Neat {
    ...
    initializePopulation(inputNodes, hiddenNodes, outputNodes) {
        for (let i = 0; i < this.populationSize; i++) {
            const genome = new player_1.Player(new neuralNetwork_1.NeuralNetwork(inputNodes, hiddenNodes, outputNodes));
    ...
    performCrossover(parent1, parent2) {
        ...
        tf.tidy(() => {
            ...
            for (let i = 0; i < this.populationSize; i++) {
                const childBrain = new neuralNetwork_1.NeuralNetwork(parentInputNodes, parentHiddenNodes, parentOutputNodes);
                childBrain.getModel().setWeights(newWeights);
                const child = new player_1.Player(childBrain);
    ...
    mutate(child) {
        ...
        tf.tidy(() => {
            for (let i = 0; i < weightMatrices.length; i++) {
                ...
                const matrixValues = currentMatrix.clone();
                const mutableValues = matrixValues.arraySync();
